...
let pp_header fmt (h : Pathfinder.Encoder.packet_header) =
  Fmt.pf fmt "%-24s  %-24s  %s"
    (show_endpoint h.src_ip  h.src_port)
    (show_endpoint h.dest_ip h.dest_port)
    (show_protocol h.protocol)

let pp_examples fmt (examples : Pathfinder.Diff.packet_examples) =
  match examples with
...
let pp_prefix fmt (d : Pathfinder.Diff.packet_diff) =
...
let pp_packet_diff fmt (d : Pathfinder.Diff.packet_diff) =
